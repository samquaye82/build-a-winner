...
- **Scoring** (`scoring.ts`, weights agreed with Sam): Squad Quality 35% (0.7 XI avg quality + 0.3 best-10 depth, zero-padded), Balance 25% (coverage vs template GK 3, RB 2, LB 2, CB 4, CM 4, AM 2, RW 2, LW 2, ST 2), Age profile 20% (band scores, peak 21–28), Contract health 15% (remaining-months scores, quality-weighted), Value created 5% (final worth vs handed-over worth, ±20% spans 0–100). `PICK_XI` (final window only, re-pickable) stores the XI in state so replays can rescore; formations live in `formations.ts`.
